...
object AndroidNdk {
  ...
  lazy val defaultSettings: Seq[Setting[_]] = inConfig(Android) (Seq (
    ...
    libsDirectoryName := DefaultLibsDirectoryName,
    ndkEnvs := DefaultEnvs
    ))
  ...
  private def ndkBuildTask(targets: String*) = (ndkBuildPath, nativeOutputPath, nativeLibrariesPath) map {(ndkBuildPath, obj, libs) =>
  ...
  lazy val settings: Seq[Setting[_]] = defaultSettings ++ pathSettings ++ inConfig(Android) (Seq (
    ...
    //cleanFiles <+= (nativeLibrariesPath in Android).identity
    clean <<= (clean, ndkClean in Android) map { (clean, ndkClean) => ndkClean ; clean  }  
  )
}
  /* FIXME: Make compile depend on ndkBuild. */
